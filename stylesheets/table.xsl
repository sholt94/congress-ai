...
<!-- =============================================================
	...
	<!--  DEVELOPER: Alia Malhas/Tanya Braginsky /Aikya mareddy        -->
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="checkCodificationStyle">
		<xsl:choose>
			<xsl:when
		...
		<xsl:element name="{name()}">
			<xsl:attribute name="style">
				<xsl:choose>
					...
					<xsl:when
						test="  not(local-name(ancestor::section[1]/child::*[position()=last()]) = 'quoted-block') and not (local-name(ancestor::section[1]/child::*[1]/child::*[position()=last()]) = 'quoted-block' and count(ancestor::section[1]/child::*)=1) and not(local-name(ancestor::section[1]/child::*[position()=last()]/child::*[position()=last()]) = 'quoted-block')">
	...
	<xsl:template match="//thead//entry">
		<td>
				...
				<xsl:if test="count(preceding-sibling::entry) = 0">
					<xsl:if test="count(../preceding-sibling::row) = 0">
						<xsl:if test="ancestor::quoted-block">
							<xsl:choose>
								<xsl:when test="ancestor::quoted-block/@other-style='codification-not-quoted'">
									<xsl:if
								...
								<xsl:otherwise>
									<xsl:if
										test="string-length(normalize-space(.)) &gt; 0 and string-length(ancestor::table/ttitle) = 0 and string-length(ancestor::table/tdesc) = 0">								
										<xsl:choose>
											<xsl:when test="local-name(child::*[1])='bold'">
												<b>
													<xsl:text disable-output-escaping="yes">“</xsl:text>	
												</b>
											</xsl:when>
			...
			<xsl:if test="(starts-with(ancestor::table/@table-type, 'subformat'))">
				...
				<xsl:if test="count(preceding-sibling::entry) = 0">
					<xsl:if test="count(../preceding-sibling::row) = 2">
						<xsl:if test="string-length(../preceding-sibling::row[2]/entry[1]) = 0">
							<xsl:if test="ancestor::quoted-block">
								<xsl:if
									test="string-length(normalize-space(.)) &gt; 0 and string-length(ancestor::table/ttitle) = 0 and string-length(ancestor::table/tdesc) = 0">
									<xsl:text disable-output-escaping="yes">“</xsl:text>
	...
	<xsl:template name="nestedQBTable">
		<xsl:choose>
	...
	<xsl:template match="//tbody//entry">
		...
		<xsl:variable name="isNestedQBTable">
			<xsl:call-template name="nestedQBTable"/>
		</xsl:variable>
		<td>
			<xsl:variable name="currColname">
				<xsl:if test="@colname">
			...
			<span>
				...
				<xsl:choose>
					...
					<xsl:when test="ancestor::quoted-block and $isFirstNonEmptyEntry = 'yes'">
						<xsl:choose>
							<xsl:when test="count(preceding-sibling::entry) = 1 and count(../preceding-sibling::row)=0">
							...
							<xsl:when test="count(preceding-sibling::entry) = 0 and count(../preceding-sibling::row)=0">
								<xsl:if test="string-length(preceding-sibling::entry) = 0 and string-length(current())>0 and
									string-length(ancestor::table/ttitle) = 0 and string-length(ancestor::table/tdesc) = 0 and string-length(ancestor::table/tgroup/thead) = 0">
									<xsl:text disable-output-escaping="yes">“</xsl:text>
								</xsl:if>
							</xsl:when>
						</xsl:choose>
				...
				<xsl:if test="ancestor::quoted-block and ancestor::quoted-block/after-quoted-block">
					...
					<xsl:variable name="isCodificationStyle">
						<xsl:call-template name="checkCodificationStyle"/>
					</xsl:variable>
					<xsl:variable name="qbdata">
						...
						<xsl:choose>
							...
							<xsl:when test="ancestor::quoted-block and local-name(following::*[1])!='after-quoted-block'">
					...
					<xsl:choose>
						...
						<xsl:otherwise>
									<xsl:if test="count(following-sibling::entry) = 0">
										<xsl:if test="count(../following-sibling::row) = 0">
											<xsl:choose>
												<xsl:when test="local-name(following::*[1])!='after-quoted-block' and $isCodificationStyle='yes' and ancestor::quoted-block/@other-style='codification-quoted'">	
												</xsl:when>
												<xsl:when test="local-name(following::*[1])!='after-quoted-block' and $isNestedQBTable='yes' and  $isCodificationStyle='no'">	
												</xsl:when>
												<xsl:otherwise>
													<xsl:value-of select="$qbdata"/>
												</xsl:otherwise>
